...
class Voc2012ClassificationTest(data.Dataset):
    ...
    def get_number_classes(self):
        return len(self.classes)
def read_object_labels_all(root):
    dataset="VOC2012"
    set="trainval"
    path_labels = os.path.join(root, 'VOCdevkit', dataset, 'ImageSets', 'Main')
    labeled_data = dict()
    ...
    for i in range(num_classes):
        ...
        if i == 0:
            ...
            for (name, label) in data.items():
                labeled_data[name][i] = label           
    return labeled_data
class VocAll(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None):  
        #, inp_name=None, adj=None):
        self.root = root
        self.path_devkit = os.path.join(root, 'VOCdevkit')
        self.path_images = os.path.join(root, 'VOCdevkit', 'VOCall', 'JPEGImages')
        self.set = set
        self.transform = transform
        self.target_transform = target_transform

        # download dataset
        #download_vocall(self.root)

        # define path of csv file
        path_csv = os.path.join(self.root,'VOCdevkit', 'files', 'VOCall')
        # define filename of csv file
        file_csv = os.path.join(path_csv, 'classification_' + set + '.csv')
        ...
        if not os.path.exists(file_csv):
            if not os.path.exists(path_csv):  # create dir if necessary
                os.makedirs(path_csv)
            # generate csv file
            labeled_data = read_object_labels_all(self.root)
            # write csv file
            write_object_labels_csv(file_csv, labeled_data)
        ...
        self.inp_name = inp_name
        '''
        print('[dataset] VOC ALL classification set=%s number of classes=%d  number of images=%d' % (
            set, len(self.classes), len(self.images)))
